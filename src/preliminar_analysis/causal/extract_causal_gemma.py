...
EVENT_DIR = Path("data/preliminar_analysis/event/gemma")
OUTPUT_DIR = Path("data/preliminar_analysis/causal/gemma")
...
class Inferencer:
    ...
    def __call__(self, prompt: str) -> str:
        ...
        inputs = {
            key: value.to(self.model.device, dtype=self.model.dtype)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Causal inference Gemma-3n-E4B dagli eventi consolidati.")
    ...
    for index, path in enumerate(files, 1):
        ...
        print(f"[{index}/{len(files)}] {video_id} su CUDA:1")
