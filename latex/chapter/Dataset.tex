...
\end{lstlisting}
Each file must contain the columns \texttt{titolo\_video}, \texttt{macroarea}, and \texttt{risposta}. For backward compatibility, the alternative column name \texttt{risposte} is also accepted and automatically renamed. Annotator identifiers are extracted directly from the filenames:
\begin{lstlisting}[language=Python]
...
\end{lstlisting}
The final dataset is saved to \texttt{data/vsv/caption-foil/caption\_foil.csv}. Alongside the randomized options and target labels, the output retains all metadata required to trace each pair back to its originating video, question, annotator, and human response:
\begin{lstlisting}[language=Python]
columns = [
    "id",
    "video_id",
