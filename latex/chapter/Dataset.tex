...
\begin{quote}
\small
\texttt{Data una caption in italiano (C) riguardante la posizione o la localizzazione di qualcuno o qualcosa, il tuo compito è creare il suo foil (F) modificando solo l'informazione spaziale.}\\
...
\begin{quote}
\small
\texttt{Data una caption in italiano (C) riguardante gli eventi e quando avvengono, il tuo compito è creare il suo foil (F) modificando solo l'informazione temporale.}\\
...
\paragraph{Implementation Details}
The \textit{caption--foil} generation pipeline is implemented in \texttt{caption\_foil.py}. The script relies on \texttt{pandas} for tabular data processing and the OpenAI Python client for model inference. Both caption and foil generation utilize \texttt{gpt-4o-2024-08-06}, with authentication managed via the \texttt{OPENAI\_API\_KEY} environment variable. Input and output paths, along with the expected annotation structure, are defined at initialization:
\begin{lstlisting}[language=Python]
